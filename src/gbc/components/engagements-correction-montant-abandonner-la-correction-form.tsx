...
const EngagementsCorrectionMontantAbandonnerLaCorrectionForm: FunctionComponent = () => {
  ...
  const [disableSupprimerBonDAnnualtion, setDisableSupprimerBonDAnnulation] = useState<boolean>(true);
  ...
  const initFormEng = () => {
    ...
    setDisableSupprimerBonDAnnulation(true);
  ...
  const [engagementASupprimer, setEngagementASupprimer] = useState<any>({});
    ...
    EngagementEN01ViewService.getBonDAnnulationModifiables(Number(gestionCourante), Number(idBudget)).then(data => {
      ...
      if (result.length !== 0) {
        EngagementEN01ViewService.getByNumBe(result[0].idEng).then(engParent => {
          ...
          setDisableSupprimerBonDAnnulation(false);

          setEngagementASupprimer(result[0]);
          ...
          setFormEng({
            ...
            disponible: { value: engParent.dispoAvant },//
  ...
  ///////////////// GESTION SUPPRIMER BON D'ANNULATION
  const handleSupprimerBonDAnnulationButtonClick = () => {
  ...
  const supprimerBonDAnnulation = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        EngagementService.delete(engagementASupprimer.numBe).then(res => {
          PieceJustifService.deleteByNumBe(engagementASupprimer.numBe).then(data => {
            rechercherAfficherEngagement(engagementASupprimer.benum);
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">ENGAGEMENTS &gt; CORRECTION MONTANT &gt; ABANDON DE BON D'ANNULATION OU DE DEGAGEMENT</h6>
            <Form>
              <Card className="mb-1">
                <Card.Body className='p-1'>
                  <Form.Group as={Row}>
                    ...
                    <ButtonGroup as={Col} xs={4} size="sm" className='justify-content-end'>
                      <Button variant="outline-danger" title="Supprimer bon d'annulation ou de dégagement" style={{maxWidth:"65px", maxHeight:"30px"}} onClick={ () => handleSupprimerBonDAnnulationButtonClick()} disabled={disableSupprimerBonDAnnualtion}><DeleteIcon /></Button>
                    </ButtonGroup>
                  </Form.Group>
                </Card.Body>
              </Card>
...
export default EngagementsCorrectionMontantAbandonnerLaCorrectionForm;
