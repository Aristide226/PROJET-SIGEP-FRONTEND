...
const LiquidationDemandeDeLiquidationModifierRecuForm: FunctionComponent = () => {
  ...
  const handleSubmitFormReçuDemandeDeLiquidation = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formReçuDemandeDeLiquidation.numBe.value === rdlAModifier.numBe) {
      // L'ENGAGEMENT N'A PAS CHANGE
      if (removeNonNumeric(formReçuDemandeDeLiquidation.montant.value) > Number(formReçuDemandeDeLiquidation.resteALiquiderEngagement.value) + Number(rdlAModifier.montant)) {
        okWarnignDialog("Montant du reçu doit etre inférieur ou égal au reste à liquider de l'engagement concerné ! " + (Number(formReçuDemandeDeLiquidation.resteALiquiderEngagement.value) + Number(rdlAModifier.montant)).toLocaleString())
        return;
      }
    } else {
      // L'ENGAGEMENT A CHANGE
      if (removeNonNumeric(formReçuDemandeDeLiquidation.montant.value) > Number(formReçuDemandeDeLiquidation.resteALiquiderEngagement.value)) {
        okWarnignDialog("Montant du reçu doit etre inférieur ou égal au reste à liquider de l'engagement concerné ! " + Number(formReçuDemandeDeLiquidation.resteALiquiderEngagement.value).toLocaleString())
        return;
      }
    }
  ...
  const editReçu = () => {
    ...
    newReçu.montDjaLiq = formReçuDemandeDeLiquidation.dejaLiquideEngagement.value;
    ...
    if (rdlAModifier.numMand === null) {
      // MODIFICATION D'UN NOUVEAU RECU SANS MANDAT : ON MODIFIE UNIQUEMENT LE RECU
      LiquidationService.edit(rdlAModifier.codLiq, newReçu).then(reçu => {
        ...
        PieceJustifService.addPiecesJustificativesRDL(reçu.codLiq, pieceJustifs).then(res => {
          if (res) {
            ...
            setDisableAjouterUnePieceJustificative(true);
            setDisableRappelPiecesJustificatives(true);
            rechercherAfficherRdlApresModification(reçu.numBl);
            okSuccessDialog("Reçu modifé avec succès !");
          }
        })
      });
    } else {
      // MODIFICATION D'UN RECU AVEC MANDAT Etat = MO OU M3 : ON MODIFIE LE RECU PUIS SON MANDAT
      LiquidationService.edit(rdlAModifier.codLiq, newReçu).then(reçu => {
        let newMand: MandatRequestDto = emptyMandatRequestDto;
        newMand.dateMand = rdlAModifier.dateMand;
        newMand.montant = removeNonNumeric(formReçuDemandeDeLiquidation.montant.value);
        newMand.dateEtat = (rdlAModifier.etatMand ==="M3")? ((isGestionClose)? dateEnregistrementPourGestionClose : new Date()) : rdlAModifier.dateEtatMand;
        newMand.datePaie = rdlAModifier.datePaieMand;
        newMand.montantPaie = rdlAModifier.montantPaieMand; 
        newMand.idCompte = rdlAModifier.idCompteMand; 
        newMand.idUser = utilisateurCourante;
        newMand.datePosition = rdlAModifier.datePositionMand;
        newMand.benum = formReçuDemandeDeLiquidation.benum.value;
        newMand.objet = formReçuDemandeDeLiquidation.natDepense.value;
        newMand.montEngage = reçu.montEngage; 
        newMand.montDjaLiq = reçu.montDjaLiq;
        newMand.dateEtatBl = (rdlAModifier.etatMand ==="M3")? ((isGestionClose)? dateEnregistrementPourGestionClose : new Date()) : rdlAModifier.dateEtatBlMand;
        newMand.datePec = rdlAModifier.datePecMand;
        newMand.receptionne = rdlAModifier.receptionneMand;
        newMand.genererOv = rdlAModifier.genererOvMand;
        newMand.avecReversement = rdlAModifier.avecReversementMand;
        newMand.precompte = rdlAModifier.precompteMand;
        newMand.datePrecompte = rdlAModifier.datePrecompteMand;
        newMand.userPrecompte = rdlAModifier.userPrecompteMand;
        newMand.dateValid = rdlAModifier.dateValidMand;
        newMand.idFiche = rdlAModifier.idFicheMand;
        newMand.montantTotalPec = rdlAModifier.montantTotalPecMand;
        newMand.estOrdre = rdlAModifier.estOrdreMand;
        newMand.mandNumApresVisaCf = rdlAModifier.mandNumApresVisaCf;
        newMand.idEtatTransL = (rdlAModifier.etatMand ==="M3")? 0 : rdlAModifier.idEtatTransLMand;
        newMand.idEtatTransM = (rdlAModifier.etatMand ==="M3")? 0 : rdlAModifier.idEtatTransMMand;
        newMand.idBordEmis = rdlAModifier.idBordEmisMand;
        newMand.idBord = rdlAModifier.idbordMand;
        newMand.codBud = formReçuDemandeDeLiquidation.codBud.value;
        newMand.idBudget = rdlAModifier.idBudget;
        newMand.compteFourn = rdlAModifier.compteFournMand;
        newMand.idContrat = formReçuDemandeDeLiquidation.idContrat.value;
        newMand.idFourn = formReçuDemandeDeLiquidation.idFourn.value;
        newMand.gestion = rdlAModifier.gestion;
        newMand.codLiq = rdlAModifier.codLiq;
        newMand.idLettrage = rdlAModifier.idLettrageMand;
        newMand.idModePaie = rdlAModifier.idModePaieMand;
        newMand.numBe = formReçuDemandeDeLiquidation.numBe.value;
        newMand.etat = (rdlAModifier.etat ==="M3")? "M0" : rdlAModifier.etatMandMand;
        newMand.etatBl = (rdlAModifier.etat ==="M3")? "L0" : rdlAModifier.etatBlMand;
        newMand.numMandParent = rdlAModifier.numMandParent;

        MandatService.edit(rdlAModifier.numMand, newMand).then(mandat => {
          // ON ENREGISTRE LES PIECES JUSTIFICATIVES DANS PieceJustif
          let pieceJustifs: PieceJustifRequestDto[] = [];
          pjSelectionnees.forEach(item => {
            pieceJustifs.push({
  ...
  const rechercherAfficherRdlApresModification = (numBl: number) => {
    LiquidationViewService.getReçuModifiables(Number(gestionCourante), Number(idBudget)).then(data => {
      const result = data.filter(item => {
        return (item.numBl === numBl)
      })

      if (result.length !== 0) {
        // ON POUSSE LES DONNEES DANS FORMULAIRE
        setFormReçuDemandeDeLiquidation({
  ...
  const [rdlAModifier, setRdlAModifier] = useState<any>({});
  ...
  useEffect(() => {
    formReçuDemandeDeLiquidation.nombrePj.value = pjSelectionnees.length;
  }, [pjSelectionnees])
  ///////////////// GESTION PIECES JUSTIFICATIVES SELECTIONNEES POUR LE RDL
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">LIQUIDATION &gt; DEMANDE DE LIQUIDATION &gt; MODIFICATION DU RE&Ccedil;U DE DEMANDE DE LIQUIDATION</h6>
                ...
                <Card.Body className='p-1'>
                  <DataTable
                    ...
                    onRowClicked={ (row, e) => {
                      ...
                      formReçuDemandeDeLiquidation.codBud.value = row.codBud;
...
export default LiquidationDemandeDeLiquidationModifierRecuForm;
