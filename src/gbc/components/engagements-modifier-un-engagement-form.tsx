...
import MotifsRejetDossierService from '../services/motifs-rejet-dossier-servic';
...
const EngagementsModifierUnEngagementForm: FunctionComponent = () => {
  ...
  const [disableDateCreation, setDisableDateCreation] = useState<boolean>(true);
  ...
  const [borderColorDateCreation, setBorderColorDateCreation] = useState<string>("");
  ...
  const handleInputChangeFormEng = (e: any): void => {
    ...
    if (fieldName === "montant") {
      ...
      if (engagementAModifier.codBud !== formEng.codBud.value) {
        ...
        if (removeNonNumeric(fieldValue)> engagementAModifier.montant + formEng.disponible.value) {
          fieldValue = addSepartor(removeNonNumeric(engagementAModifier.montant + formEng.disponible.value));
          okWarnignDialog("Montant de l'engagement doit etre inférieur ou égal au montant de l'engagement à modifier + le disponible réel !")
        }        
      }
    }
  ...
  const validateFormEng = () => {
    ...
    // Date de création
    if(new Date(formEng.dateCreation.value) < dateDeCreationMinimm || new Date(formEng.dateCreation.value) > dateDeCreationMaximum) {
      const errorMsg: string = 'La date doit etre compris entre 01/01/'+ gestionCourante + ' et ' + '31/12/' + gestionCourante + ' !';
      const newField: Field = { value: formEng.dateCreation.value, error: errorMsg, isValid: false };
      newForm = { ...newForm, ...{ dateCreation: newField } };
      setBorderColorDateCreation("red")
    } else {
      const newField: Field = { value: formEng.dateCreation.value, error: '', isValid: true };
      newForm = { ...newForm, ...{ dateCreation: newField } };
      setBorderColorDateCreation("")
    }
    ...
    if(formEng.idFourn.value === "") {
      ...
      newForm = { ...newForm, ...{ idForun: newField } };
    ...
    setFormEng(newForm);
    return newForm.dateCreation.isValid && newForm.codBud.isValid  && newForm.idFourn.isValid && newForm.proced.isValid && newForm.montant.isValid && newForm.natDepense.isValid;
  }
  ...
  const editEng = () => {
    ...
    newEng.dateCreation = formEng.dateCreation.value;
    ...
    newEng.dateEtat = (engagementAModifier.etat ==="E3")? ((isGestionClose)? dateEnregistrementPourGestionClose : new Date()) : formEng.dateEtat.value;
    ...
    newEng.receptionne = (engagementAModifier.etat ==="E3")? false : formEng.receptionne.value;
    newEng.dateValid = formEng.dateValid.value;
    newEng.codLiq = formEng.codLiq.value;
    newEng.codBord = formEng.codBord.value;
    newEng.idContrat = formEng.idContrat.value;
    newEng.idEngParent = formEng.idEngParent.value; 
    newEng.etat = (engagementAModifier.etat ==="E3")? "E0" : formEng.etat.value;
    newEng.idEtatTrans = (engagementAModifier.etat ==="E3")? 0 : formEng.idEtatTrans.value;
    newEng.proced = formEng.proced.value;
    newEng.codBud = formEng.codBud.value;
    newEng.idBudget = formEng.idBudget.value;
    newEng.idFourn = formEng.idFourn.value;
    newEng.gestion = formEng.gestion.value;
    
    if (engagementAModifier.proced ==='EN01') {
      // MODIFICATION D'UN ENGAGEMENT EN PROCEDURE NORMALE
      if (formEng.proced.value ==='EN01') {
        // LA PROCEDURE RESTE NORMALE : ON MODIFIE JUSTE L'ENGAGEMENT
        EngagementService.edit(formEng.numBe.value, newEng).then(engagement => {
          ...
          PieceJustifService.addPiecesJustificatives(engagement.numBe, pieceJustifs).then(res => {
            if (res) {
              if (engagementAModifier.etat ==="E3") MotifsRejetDossierService.setActifToFalse(engagement.numBe);
              ...
              getEngagements(); // Pour actu ls engagements modifiables
              getContrats() //Pour actu les contrats
              rechercherAfficherEngagement(engagementAModifier.benum, engagementAModifier.proced); 
              setDisableEditerEngagment(false);
              okSuccessDialog("Engagment modifiée avec succès !"); 
            }
          })
        })
      } else {
        // LA PROCEDURE PASSE A SIMPLIFIEE : ON MODIFIE L'ENGAGEMENT PUIS ON CREE SON MANDAT
        EngagementService.edit(formEng.numBe.value, newEng).then(engagement => {
          let newMand: MandatRequestDto = emptyMandatRequestDto;
          newMand.dateMand = engagement.dateCreation;
          newMand.montant = engagement.montant;
          newMand.dateEtat = engagement.dateCreation;
          ...
          newMand.dateEtatBl = engagement.dateCreation; 
          ...
          newMand.dateValid = engagement.dateCreation;
          newMand.idFiche = 0;
          newMand.montantTotalPec = 0;
          newMand.dateCreat = engagement.dateCreation
          ...
          newMand.compteFourn = (formEng.compteFourn.value !=="")? formEng.compteFourn.value: null;
      ...
      // MODIFICATION D'UN ENGAGEMENT EN PROCEDURE SIMPLIFIEE
      if (formEng.proced.value ==='EN02') {
        // LA PROCEDURE RESTE SIMPLIFIEE : ON MODIFIE L'ENGAGEMENT PUIS SON MANDAT
        EngagementService.edit(formEng.numBe.value, newEng).then(engagement => {
          let newMand: MandatRequestDto = emptyMandatRequestDto;
          newMand.dateMand = engagement.dateCreation;
          newMand.montant = engagement.montant;
          newMand.dateEtat = (engagementAModifier.etat ==="E3")? ((isGestionClose)? dateEnregistrementPourGestionClose : new Date()) : engagementAModifier.dateEtatMand;
          newMand.datePaie = engagementAModifier.datePaie;
          newMand.montantPaie = engagementAModifier.montantPaie; //???
          newMand.idCompte = engagementAModifier.idCompte; // ???
          newMand.idUser = utilisateurCourante;
          newMand.datePosition = engagementAModifier.datePosition;
          newMand.benum = engagementAModifier.benum;
          newMand.objet = engagement.natDepense;
          newMand.montEngage = engagementAModifier.montEngage; // ???
          newMand.montDjaLiq = engagementAModifier.montDjaLiq;
          newMand.dateEtatBl = (engagementAModifier.etat ==="E3")? ((isGestionClose)? dateEnregistrementPourGestionClose : new Date()) : engagementAModifier.dateEtatBl;
          newMand.datePec = engagementAModifier.datePec;
          newMand.receptionne = (engagementAModifier.etat ==="E3")? false : engagementAModifier.receptionneMand;
          newMand.genererOv = engagementAModifier.genererOv;
          newMand.avecReversement = engagementAModifier.avecReversement;
          newMand.precompte = engagementAModifier.precompte;
          newMand.datePrecompte = engagementAModifier.datePrecompte;
          newMand.userPrecompte = engagementAModifier.userPrecompte;
          newMand.dateValid = engagementAModifier.dateValidMand;
          newMand.idFiche = engagementAModifier.idFiche;
          newMand.montantTotalPec = engagementAModifier.montantTotalPec;
          newMand.estOrdre = formEng.estOrdre.value;
          newMand.mandNumApresVisaCf = engagementAModifier.mandNumApresVisaCf;
          newMand.idEtatTransL = (engagementAModifier.etat ==="E3")? 0 : engagementAModifier.idEtatTransL;
          newMand.idEtatTransM = (engagementAModifier.etat ==="E3")? 0 : engagementAModifier.idEtatTransM;
          newMand.idBordEmis = engagementAModifier.idBordEmisMand;
          newMand.idBord = engagementAModifier.idbordMand;
          newMand.codBud = engagement.codBud;
          newMand.idBudget = engagement.idBudget;
          newMand.compteFourn = (formEng.compteFourn.value !=="")? formEng.compteFourn.value: null;
          newMand.idContrat = engagement.idContrat;
          ...
          newMand.gestion = engagement.gestion;
          newMand.codLiq = engagement.codLiq;// ???
          newMand.idLettrage = engagementAModifier.idLettrage;
          newMand.idModePaie = engagementAModifier.idModePaie;
          newMand.numBe = engagement.numBe;
          newMand.etat = (engagementAModifier.etat ==="E3")? "M0" : engagementAModifier.etatMand;
          newMand.etatBl = (engagementAModifier.etat ==="E3")? "L0" : engagementAModifier.etatBl;
          newMand.numMandParent = engagementAModifier.numMandParent;
          
          MandatService.edit(engagementAModifier.numMand, newMand).then(mandat => {
            // ON ENREGISTRE LES PIECES JUSTIFICATIVES DANS PieceJustif
            let pieceJustifs: PieceJustifRequestDto[] = [];
            pjSelectionnees.forEach(item => {
              pieceJustifs.push({
            ...
            PieceJustifService.addPiecesJustificatives(engagement.numBe, pieceJustifs).then(res => {
              if (res) {
                ...
                rechercherAfficherEngagement(engagementAModifier.benum, engagementAModifier.proced); 
                setDisableEditerEngagment(false);
                okSuccessDialog("Engagment modiffié avec avec succès !");
              }
            })
          })
        })
      } else {
        // LA PROCEDURE PASSE A NORMALE : ON MODIFIE L'ENGAGEMENT PUIS SUPPRIME SON MANDAT
        EngagementService.edit(formEng.numBe.value, newEng).then(engagement => {
          MandatService.delete(engagementAModifier.numMand).then(res => {
            // ON ENREGISTRE LES PIECES JUSTIFICATIVES DANS PieceJustif
            let pieceJustifs: PieceJustifRequestDto[] = [];
            pjSelectionnees.forEach(item => {
              pieceJustifs.push({
  ...
  const getCompteDestinataires_ = (idDest: number) => {
    CompteDestinataireService.getByDestinataires(idDest).then(data => {
      setCompteDestinataires(data);
    })
  }
    ...
    if (proced === "EN01") {
      EngagementEN01ViewService.getEngagementModifiables(Number(gestionCourante), Number(idBudget)).then(data => {
        ...
        if (result.length !== 0) {
          ...
          // CREE A PARTIR D'UN RDL
          if (result[0].apartirDemandLiq === true) {
            contratDecoche();
            setDisableSelectionnerRDL(false);
            setDisableGenererRDL(false);
          } 
          
          // CREE A PARTIR D'UN CONTRAT
          if (result[0].idContrat !== null && result[0].apartirDemandLiq === false) {
            RDLDecoche();
          ...
          setFormEng({
            ...
            dotInitiale: { value: result[0].dot_Initiale },//
          ...
          // RECUPERATION DES COMPTES COMPTES DESTINATAIRE DU BENEFICIAIRE APRES QUE compteFourn est initialisé avec le compte selectionné
          getCompteDestinataires_(result[0].idFourn);
          
          // RECUPERATION DES PIECES JUSTIFICATIVES
          getPjSelectionnees(result[0].numBe);
    
          // ACTIVER OU DESACTIVER EN FONCTION DE PROCED
          if (result[0].proced ==="EN01") {
            setDisableAvecDecision(true);
            setDisableCorrigerLaDecision(true);
            setDisableAuProfitDuCompte(true);
            setDisableEffacerAuProfitDuCompte(true);
      ...
      EngagementEN02ViewService.getEngagementModifiables(Number(gestionCourante), Number(idBudget)).then(data => {
        ...
        if (result.length !== 0) {
          ...
          setFormEng({
            ...
            ifuMle: { value: result[0].ifumle }, //
            nom: { value: result[0].nom }, //
            compteFourn: { value: (result[0].compteFourn !== null)? result[0].compteFourn : '' },//
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">ENGAGEMENTS &gt; MODIFIER UN ENGAGEMENT</h6>
                              ...
                              sx={{ 
                                "& .MuiOutlinedInput-root": 
                                  { 
                                    ...
                                    height: 30, 
                                    padding: 1,
                                    border: '1px solid ' + borderColorDateCreation
                                  }
                              }}
                            />
                          ...
                          <Row style={{fontSize:"0.8em"}}>
                            <Col>
                              <Form.Group controlId="proced" as={Row} className='mb-1'>
                                ...
                                <Col>
                                  <Form.Select name='proced' value={formEng.proced.value} size="sm" onChange={e => handleInputChangeFormEng(e)} disabled={disableProceed} style={{ borderColor: borderColorProced}}>
                              ...
                              <Form.Group controlId="montant" as={Row} className='mb-1'>
                                ...
                                <Col><Form.Control name='montant' value={formEng.montant.value} onChange={e => handleInputChangeFormEng(e)} size='sm' type="text" style={{ borderColor: borderColorMontant}} disabled={disableMontantEngagement} /></Col>
                              </Form.Group>
                            </Col>
                            <Col>
                              <Form.Group controlId="natDepense" as={Row} className='mb-1'>
                                <Col xs={4}><Form.Label>Object :</Form.Label></Col>
                                <Col><Form.Control as="textarea" rows={2} name='natDepense' value={formEng.natDepense.value} title={formEng.natDepense.value} size='sm' disabled={disableNatDepense} style={{ borderColor: borderColorObjet}} onChange={e => handleInputChangeFormEng(e)}/></Col>
                              </Form.Group>
              ...
              <Card>
                <Card.Body className='p-1'>
                  <DataTable
                    ...
                    striped
                    onRowClicked={ (row, e) => {
                      if (formEng.codBud.value !== row.codBud) { // EXECUTE SI ON CHANGE DE LIGNE BUDGETAIRE
                        if (removeNonNumeric(formEng.montant.value) <= row.disponible) {
                          ...
                          formEng.disponible.value = row.disponible;
                ...
                <Card.Body className='p-1'>
                  <DataTable
                    ...
                    if (row.resteAEngager <= formEng.dispoAvant.value) {
                        ...
                        formEng.engageContrat.value = (engagementAModifier.idContrat === row.idContrat)? row.engage - engagementAModifier.montant : row.engage;
                        formEng.resteAEngagerContrat.value = (engagementAModifier.idContrat === row.idContrat)? row.montant - (row.engage - engagementAModifier.montant) : row.montant - row.engage;
                        formEng.montant.value = (engagementAModifier.idContrat === row.idContrat)? (row.montant - (row.engage - engagementAModifier.montant)).toLocaleString() : (row.montant - row.engage).toLocaleString();
                        formEng.auProfit.value = 'F';
                        formEng.idFourn.value = row.idFourn;
                        formEng.ifuMle.value = row.ifumle;
                        formEng.nom.value = row.nom;
...
export default EngagementsModifierUnEngagementForm;
