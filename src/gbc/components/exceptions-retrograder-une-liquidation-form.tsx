...
import { emptyInfosPourRetrograderLiquidation, InfosPourRetrograderLiquidation } from '../models/infos-pour-retrograder-liquidation';
...
const ExceptionsRetrograderUneLiquidationForm: FunctionComponent = () => {
  ...
  ///////////////// GESTION LIQUIDATION A RETROGRADER
  ...
  const getLiquidations = () => {
    LiquidationViewService.getLiquidationRetrogradables(Number(gestionCourante), Number(idBudget), utilisateurCourante).then(data => {
  ...
  ///////////////// GESTION LIQUIDAIONS A RETROGRADER
  ...
  ///////////////// GESTION RETROGRADER LIQUIDATION
  const handleValiderLiquidationButtonClick = () => {
    ...
    let infosPourRetrograderLiquidation: InfosPourRetrograderLiquidation = emptyInfosPourRetrograderLiquidation;
    infosPourRetrograderLiquidation.ids = ids;
    infosPourRetrograderLiquidation.date = (isGestionClose)? dateEnregistrementPourGestionClose : new Date();
    infosPourRetrograderLiquidation.idUser = utilisateurCourante;      

    MandatService.retrograderLiquidation(infosPourRetrograderLiquidation).then(res => {
      if (res) {
        setSelectedRowIds([])
        getLiquidations();
        okSuccessDialog("Liquidation retrogradée(s) avec succès !");
      }
    })
  }
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">EXCEPTIONS &gt; RETROGRADER UNE LIQUIDATION</h6>
            <Form>
              <Card className="mb-1">
...
export default ExceptionsRetrograderUneLiquidationForm;
