...
const EngagementsCorrectionMontantCreerUnBonDAnnulationForm: FunctionComponent = () => {
  ...
  const handleInputChangeFormEng = (e: any): void => {
    ...
    if (fieldName ==="montantNouveauEng") {
      ...
      } else if (Number(removeNonNumeric(fieldValue)) === 0 || Number(removeNonNumeric(fieldValue)) > engagementAAnnuler.resteALiquider) {
        fieldValue = addSepartor(removeNonNumeric(engagementAAnnuler.resteALiquider));
        formEng.natDepenseNouveauEng.value = "Dégagement de " + addSepartor(removeNonNumeric(fieldValue)) + " de l'engagement \u2116 " + formEng.gestion.value + "-" + String(formEng.benum.value).padStart(4, '0')
        okWarnignDialog("Le montant doit etre supérieur à 0 et inférieur ou égal au reste à liquider !");
      }
    }
  ...
  const addEng = () => {
    ...
    newEng.dispoAvant = engagementAAnnuler.disponible;
    ...
    newEng.idEngParent = formEng.numBe.value
    ...
    EngagementService.add(newEng).then(data => {
      ...
      PieceJustifService.addPiecesJustificatives(data.numBe, pieceJustifs).then(res => {
        if (res) {
          formEng.benumNouveauEng.value = data.benum;
          formEng.dateCreationNouveauEng.value = data.dateCreation;
          getEngagements(); // Pour actu ls engagments partiellement liquidés
          setDisabledEnregistrer(true);
          setDisableEditerEngagment(false);
          okSuccessDialog("Crée avec succès !");
        }
      })
    })    
  }
  ...
  const [engagementAAnnuler, setEngagementAAnnuler] = useState<any>({});
  ...
  const tableEngagmentColumns = [
    ...
    {
      name: "\u2116",
      selector: (row: any) => String(row.benum),
      width: "50px",
      center: true
    },
    {
  ...
  const getEngagements = () => {
    EngagementEN01ViewService.getLesEngagementsPartiellementLiquides(Number(gestionCourante), Number(idBudget)).then(data => {
      setEngagements(data) 
  ...
  const rechercherAfficherEngagement = (benum: number) => {
    EngagementEN01ViewService.getLesEngagementsPartiellementLiquides(Number(gestionCourante), Number(idBudget)).then(data => {
      ...
      if (result.length !== 0) {
        ...
        setEngagementAAnnuler(result[0])
        setFormEng({
          ...
          disponible: { value: result[0].dispoAvant },//
          ...
          engageContrat: { value: result[0].engageContrat },//
          resteAEngagerContrat: { value: result[0].resteAEngagerContrat },//
          ifuMle: { value: result[0].ifumle }, //
          nom: { value: result[0].nom }, //
          dejaLiquide: { value: result[0].dejaLiquide },
          resteALiquider: { value: result[0].resteALiquider },
          montantNouveauEng: { value: addSepartor(removeNonNumeric(result[0].resteALiquider)) },
          dateCreationNouveauEng: { value: '' },
          benumNouveauEng: { value: '' },
          natDepenseNouveauEng: { value: "Dégagement de " + addSepartor(removeNonNumeric(result[0].resteALiquider)) + " de l'engagement \u2116 " + result[0].gestion + "-" + String(result[0].benum).padStart(4, '0') },
        });
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">ENGAGEMENTS &gt; CORRECTION MONTANT &gt; CREATION DE BON D'ANNULATION OU DE DEGAGEMENT</h6>
            <Form onSubmit={(e) => handleSubmitFormEng(e)}>
              <Card className="mb-1">
                <Card.Body className='p-1'>
                  <Form.Group as={Row}>
                    <InputGroup as={Col}>
                      <Form.Control name='' size='sm' value={"Engagement à annuler numero :"} type="text" className='me-1' disabled />
...
export default EngagementsCorrectionMontantCreerUnBonDAnnulationForm;
