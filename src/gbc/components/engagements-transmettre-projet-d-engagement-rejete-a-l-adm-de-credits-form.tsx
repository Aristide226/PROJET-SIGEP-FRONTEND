...
const EngagementsTransmettreProjetDEngagementRejeteALAdmDeCreditsForm: FunctionComponent = () => {
  ...
  const [selectedRowIdMotifs, setSelectedRowIdMotifs] = useState<string[]>([]);
  ...
  const getEngagements = () => {
    EngagementViewService.getEngagementRejeteAE3EtNonTransmis(Number(gestionCourante), Number(idBudget)).then(data => {
      setEngagements(data) 
  ...
  const handleEditerBordereauEngagementButtonClick = () => {
    ...
    selectedRowIds.forEach(id => {
      ...
      const result = engagements.find((item) => item.numBe === Number(id.valueOf()));
      if (result) selectedRowIdMotifs.push(result.id_motif);
    });
    ...
    bordTransmisRequestDto.dossier = "E3";
    ...
    bordTransmisRequestDto.idMotifs = selectedRowIdMotifs;
        ...
        if (res) {
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              // IMPRIMER BORDEREAU DE TRANSMISSION DE PROJETS D'ENGAGEMENT VALIDES
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">ENGAGEMENTS &gt; EDITION DE BORDEREAU DE TRANSMISSION DE PROJETS D'ENGAGEMENTS REJETES</h6>
            <Form>
              <Card className="mb-1">
...
export default EngagementsTransmettreProjetDEngagementRejeteALAdmDeCreditsForm;
