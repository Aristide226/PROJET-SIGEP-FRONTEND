...
const LiquidationDemandeDeLiquidationAnnulerRecuForm: FunctionComponent = () => {
  ...
  const [disableSupprimerLeReçu, setDisableSupprimerLeReçu] = useState<boolean>(true);
  ...
  const [rdlASupprimer, setRdlASupprimer] = useState<any>({});
  ...
  const rechercherAfficherRdl = (numBl: number) => {
    LiquidationViewService.getReçuModifiables(Number(gestionCourante), Number(idBudget)).then(data => {
      ...
      if (result.length !== 0) {
        setDisableSupprimerLeReçu(false);

        setRdlASupprimer(result[0]);
        setFormReçuDemandeDeLiquidation({
  ...
  ///////////////// GESTION SUPPRIMER RECU
  const handleSupprimerLeReçuButtonClick = () => {
    handleShowModalMotDePasseDeConnexion();
  }

  const supprimerLeReçu = () => {
    Swal.fire({
      title: 'GesBud',
      text: "Etes-vous certain de vouloir abandonner cet reçu ?",
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        LiquidationService.delete(rdlASupprimer.codLiq).then(res => {
          if (rdlASupprimer.numMand === null) {
            // LE RECU N'A PAS DE MANDAT
            PieceJustifService.deleteByCodLiq(rdlASupprimer.codLiq).then(data => {
              rechercherAfficherRdl(rdlASupprimer.codLiq);
              okSuccessDialog("Reçu abandonné avec succès !");
            })
          } else {
            // LE RECU A UN MANDAT 
            MandatService.delete(rdlASupprimer.nnumMand).then(res => {
              PieceJustifService.deleteByCodLiq(rdlASupprimer.codLiq).then(data => {
                rechercherAfficherRdl(rdlASupprimer.codLiq);
                okSuccessDialog("Reçu abandonné avec succès !");
              })              
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">LIQUIDATION &gt; DEMANDE DE LIQUIDATION &gt; ANNULATION DE RE&Ccedil;U DE DEMANDE DE LIQUIDATION</h6>
            <Form>
              <Card className="mb-1">
                <Card.Body className='p-1'>
                  <Form.Group as={Row}>
                    ...
                    <ButtonGroup as={Col} xs={2} size="sm" className='justify-content-end'>
                      <Button variant="outline-danger" title="Supprimer le réçu" className='me-1' style={{maxWidth:"65px", maxHeight:"30px"}} onClick={ () => handleSupprimerLeReçuButtonClick()} disabled={disableSupprimerLeReçu}><DeleteIcon /></Button>
                    </ButtonGroup>
                  </Form.Group>
                </Card.Body>
              </Card>
              ...
              <Card>
                <Card.Body className='p-1' style={{height: "150px"}}>
                  <DataGrid
...
export default LiquidationDemandeDeLiquidationAnnulerRecuForm;
