...
import { emptyInfosPourValiderEngagement, InfosPourValiderEngagement } from '../models/infos-pour-valider-engagement';
...
const EngagementsValiderUnEngagementForm: FunctionComponent = () => {
  ...
  const getEngagements = () => {
    EngagementViewService.getEngagementValidables(Number(gestionCourante), Number(idBudget)).then(data => {
      setEngagements(data) 
  ...
  const handleValiderEngagementButtonClick = () => {
    ...
    let infosPourValiderEngagement: InfosPourValiderEngagement = emptyInfosPourValiderEngagement;
    infosPourValiderEngagement.ids = ids;
    infosPourValiderEngagement.date = (isGestionClose)? dateEnregistrementPourGestionClose : new Date();
    infosPourValiderEngagement.idUser = utilisateurCourante;     

    EngagementService.valider(infosPourValiderEngagement).then(res => {
      if (res) {
        setSelectedRowIds([])
        getEngagements();
        okSuccessDialog("Engagments validé(s) avec succès !");
      }
    })
  }
  ...
  return (
    <Container>
          <div className="mt-1 p-1">
            <h6 className="shadow-sm text-primary text-center rounded">ENGAGEMENTS &gt; VALIDATION DE PROJET D'ENGAGEMENT</h6>
            <Form>
              <Card className="mb-1">
                <Card.Body className='p-1'>
                  <Form.Group as={Row}>
                    ...
                    <ButtonGroup as={Col} xs={1} size="sm" className='justify-content-end'>
                      <Button variant="outline-primary" title="Valider les engagements sélectionnés" className='me-1' style={{maxWidth:"65px", maxHeight:"30px"}} onClick={ () => handleValiderEngagementButtonClick()} disabled={disableValiderEngagment}><SaveRoundedIcon /></Button>
                    </ButtonGroup>
                  </Form.Group>
                </Card.Body>
              </Card>
...
export default EngagementsValiderUnEngagementForm;
