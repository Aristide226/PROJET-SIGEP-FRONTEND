...
import AccesCodeDto, { emptyAccesCodeDto } from '../models/acces-code';
import { GrhConnectedUser } from '../helpers/session-storage';
import GrhAuthenticationService from '../services/authentication-service';
import AccesCodeService from '../services/acces-codeService';
...
const GrhLogin: FunctionComponent = () => {
    ...
    const [form, setForm] = useState<Formulaire>({
      userName: { value: GrhConnectedUser() ?? ''},
      motPasse: { value: ''}
    });
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if(isFormValid) {
        ...
        GrhAuthenticationService.login(form.userName.value, form.motPasse.value).then(isAuthenticated => {
          ...
          setMessage("");
          AccesCodeService.get(form.userName.value)
          .then( data => {
            setAccesCodeDto(data);
          })
          .catch( error => {
    ...
    const handleSubmitFormulaireCMDPPD = (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if(isFormValid) {
        ...
        accesCodeDto.nbreDeFois = accesCodeDto.nbreDeFois + 1;
        accesCodeDto.motDePasse = hashedPassword;
        AccesCodeService.edit(accesCodeDto.userName, accesCodeDto).then( () => {
          handleCloseModal();
          Swal.fire({
            title: 'GesBud',
    ...
    // Code inside useEffect will execute when accesCodeDto is update
    useEffect( () => {
      if (accesCodeDto !== emptyAccesCodeDto) {
        bcrypt.compare("OK", accesCodeDto.motDePasse).then( res => {
          if (res) {
            handleShowModal();
          } else {
            accesCodeDto.nbreDeFois = accesCodeDto.nbreDeFois + 1;
    ...
    return (
        <Container>
          <Row className='mt-4'>
            <h1 className="shadow-sm text-success mt-4 p-3 text-center rounded">CONNEXION GRH</h1>
